...
public static class RXEase
{
    ...
    public static Dele Linear = (t) => {
        return t; };
    public static Dele QuadIn = (t) => {
        return t * t; };
    public static Dele QuadOut = (t) => {
        return 1f - QuadIn( 1f - t ); };
    public static Dele QuadInOut = (t) => {
        return ( t <= 0.5f ) ? QuadIn( t * 2f ) * 0.5f : QuadOut( t * 2f - 1f ) * 0.5f + 0.5f; };
    public static Dele CubeIn = (t) => {
        return t * t * t; };
    public static Dele CubeOut = (t) => {
        return 1f - CubeIn( 1f - t ); };
    public static Dele CubeInOut = (t) => {
        return ( t <= 0.5f ) ? CubeIn( t * 2f ) * 0.5f : CubeOut( t * 2f - 1f ) * 0.5f + 0.5f; };
    public static Dele BackIn = (t) => {
        return t * t * ( 2.70158f * t - 1.70158f ); };
    public static Dele BackOut = (t) => {
        return 1f - BackIn( 1f - t ); };
    public static Dele BackInOut = (t) => {
        return ( t <= 0.5f ) ? BackIn( t * 2f ) * 0.5f : BackOut( t * 2f - 1f ) * 0.5f + 0.5f; };
    public static Dele ExpoIn = (t) => {
        return Mathf.Pow( 2f, 10f * ( t - 1.0f ) ); };
    public static Dele ExpoOut = (t) => {
        return 1f - Mathf.Pow( 2f, -10f * t ); };
    public static Dele ExpoInOut = (t) => {
        return t < .5f ? ExpoIn( t * 2f ) * 0.5f : ExpoOut( t * 2f - 1f ) * 0.5f + 0.5f; };
    public static Dele SineIn = (t) => {
        return -Mathf.Cos( Mathf.PI * 0.5f * t ) + 1f; };
    public static Dele SineOut = (t) => {
        return Mathf.Sin( Mathf.PI * 0.5f * t ); };
    public static Dele SineInOut = (t) => {
        return -Mathf.Cos( Mathf.PI * t ) * 0.5f + .5f; };
    public static Dele ElasticIn = (t) => {
        return 1f - ElasticOut( 1f - t ); };
    public static Dele ElasticOut = (t) => {
        return Mathf.Pow( 2f, -10f * t ) * Mathf.Sin( ( t - 0.075f ) * ( 2f * Mathf.PI ) / 0.3f ) + 1f; };
    public static Dele ElasticInOut = (t) => {
        return ( t <= 0.5f ) ? ElasticIn( t * 2f ) / 2f : ElasticOut( t * 2f - 1f ) * 0.5f + 0.5f; };
...
public class RXTweenable
{
    ...
    public static RXTweenable DelayAction( float delay, Action action )
    {
        ...
        rt.To( 1.0f, delay, new TweenConfig().onComplete( (t) => {
            action();} ) );
        return rt;
    }
}
